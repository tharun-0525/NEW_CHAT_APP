...
from app.api import routes_user, routes_auth, routes_message, routes_ws, routes_rooms, routes_search
...
async def http_exception_handler(request: Request, exc: StarletteHTTPException):
    return JSONResponse(
        ...
        content=ResponseModel(status="failed", message=exc.detail).model_dump(),
    )
...
async def validation_exception_handler(request: Request, exc: RequestValidationError):
    ...
    return JSONResponse(
        ...
        content=ResponseModel(
            status="failed", message="Validation error", data=err
        ).model_dump(),
    )
